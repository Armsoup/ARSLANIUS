...
cls
echo ======================================================================================================================
echo                                                 Loading ARSLANIUS 22
echo         Build: %current_build%
echo         Kernel: BarOS 18.1
echo ======================================================================================================================
...
echo OS_NAME = ARSLANIUS 22 > "%reg_path%"
...
if /i "%safe_mode%"=="1" (
    set "ok=0"
    for %%a in (Help logout sysinfo support reset reboot_to_recovery report cls ver dir whoami events sfc dash fmx restore-point restore adduser deluser start regedit reboot shutdown) do (if /i "%ex_c%"=="%%a" set "ok=1")
    if "!ok!"=="0" echo [ SECURITY ] Safe mode. & goto cmd_loop
)
if /i NOT "%current_user%"=="BarOS AUTHORITY\SYSTEM" goto exec
set "ok=0"
for %%a in (Help sysinfo reset logout lock support events reboot_to_recovery report cls ver taskmgr fmx Shutdown Reboot dir adduser whoami sfc clean mail-read mail-send edit guest msg-all regedit install deluser alert restore-point restore) do (if /i "%ex_c%"=="%%a" set "ok=1")
...
echo [ STATS ] Kernel: BarOS 18.1 (V22)
...
set /p "m_txt=Message: "
set "dp="
if /i "%m_to%"=="BarOS AUTHORITY\SYSTEM" set "dp=%sys_prof%"
