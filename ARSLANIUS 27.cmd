...
choice /C 123456 /N /T %boot_timeout% /D %default_mode% /M "Select option (1-6): "
...
echo OS_NAME = ARSLANIUS 27 > "%reg_path%"
...
set "user_home=%sys_prof%"
cd /d "%user_home%" 2>nul
echo [%date% %time% INFO] BOOT_V27_INIT_%current_user% >> "%log_path%" 2>nul
cls
color 5b
echo ======================================================================================================================
...
if /i "%current_user%"=="BarOS AUTHORITY\SFC_Daemon" (
    set "ok=0"
    for %%a in (Help mv bcdboot cp rm touch edit bcdedit mkdir ls cd cat ren support reset reboot_to_recovery cls ver whoami events sfc adduser deluser regedit reboot shutdown) do (if /i "%ex_c%"=="%%a" set "ok=1")
...
echo [ STATS ] Kernel: BarOS 23.0 (V27)
echo.
echo [ SERVICES ]
...
echo. > "%root_path%\Report_v27.html"
set "report_f=%root_path%\Report_v27.html"
echo ^<html^>^<body style='background:#111;color:#0f0;font-family:monospace'^> > "%report_f%"
echo ^<h1^>ARSLANIUS 27 - SYSTEM REPORT^</h1^> >> "%report_f%"
echo ^<hr^>^<p^>Build: %current_build%^</p^> >> "%report_f%"
echo ^<p^>Kernel: 23.0^</p^> >> "%report_f%"
